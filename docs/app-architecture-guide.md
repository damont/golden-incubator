...
When an app needs to store user-uploaded files or generated assets (images, PDFs, exports, etc.), use a **storage abstraction** that supports two backends: local filesystem for development/Pi deployment and cloud blob storage for Azure deployment.

### Design

Create `api/services/storage.py` with a common interface and two implementations:
...
Initialize in `api/main.py` during lifespan:

```python
from api.services.storage import LocalStorageBackend, AzureBlobStorageBackend
...
- **For Pi deployment**, the Docker volume is sufficient. For Azure, use a Storage Account and pass the connection string as a secret through the standard Terraform/GitHub Environments pipeline.

---

## Background Workers (Redis)

When an app needs to run work outside the request/response cycle -- sending notifications, processing uploads, syncing data, running long computations -- use a **Redis-backed task queue**. The worker runs as a separate container in Docker Compose.

### Design principles

1. **Events, not commands.** Messages on Redis topics describe **what happened**, not what should happen next. For example, publish `artifact_created` with the artifact ID -- don't publish `send_artifact_notification`. This keeps the publisher decoupled from whatever downstream work exists.
2. **Clear topics after posting.** Use Redis Streams with consumer groups, not pub/sub. Streams persist messages until acknowledged, and the worker marks them done after processing. Unprocessed messages survive worker restarts.
3. **One worker container.** A single Python worker process listens on all topics and dispatches to handler functions. Don't run a separate container per topic -- that's overkill for our scale.
...
|-------------|-------|-------|
| **Local dev** | `redis:7-alpine` container in Docker Compose | Just works, no setup needed |
| **Pi (Path A)** | Same `redis:7-alpine` in Docker Compose | Redis is lightweight, runs fine on Pi. Volume persists data across restarts |
| **Azure (Path B)** | Azure Cache for Redis (Basic C0 tier) or a Redis container in the ACI group | For low-traffic apps, a Redis container in the same ACI group is cheapest. For production scale, use Azure Cache for Redis and pass the connection string as a secret |
...
- **Keep payloads minimal.** Include IDs and metadata. The handler can fetch the full object from the database if it needs more.
- **The API never waits for the worker.** Publish the event and return the response. The worker processes it asynchronously.
- **The worker shares the same codebase** (same Docker image, same Beanie models) -- it just runs a different entrypoint.
- **Add `redis[hiredis]` to `pyproject.toml`** when an app needs background workers. Don't add it speculatively.
- **Only add a worker when you actually need async processing.** Most CRUD apps don't need one. Signs you need a worker: long-running operations blocking API responses, fan-out notifications, scheduled jobs, or processing pipelines.
...
### If the app stores files

- [ ] Create `api/services/storage.py` with `StorageBackend` abstraction
- [ ] Add `STORAGE_BACKEND`, `UPLOAD_DIR` to `api/config.py` and `.env.example`
- [ ] Mount a Docker volume for uploads in `docker-compose.yml`
- [ ] If Azure deployment: add `azure-storage-blob[aio]` to `pyproject.toml` and wire `AZURE_STORAGE_CONNECTION_STRING` through Terraform/GitHub Secrets
- [ ] Verify uploads persist across container restarts locally

### If the app needs background workers

- [ ] Add `redis[hiredis]` to `pyproject.toml`
- [ ] Create `api/services/events.py` with `publish()` function
- [ ] Create `worker/main.py` with consumer group loop
- [ ] Add `redis` and `worker` services to `docker-compose.yml`
- [ ] Wire `REDIS_URL` into API and worker environment
- [ ] Verify events are published and processed end-to-end
- [ ] If Azure deployment: provision Azure Cache for Redis or add Redis container to ACI group
